...
export default function Goals() {
  ...
  useEffect(() => {
    ...
    const remaining = targetCalories - totalCaloriesBurnt;
    setRemainingCalories(`Remaining Calories to Burn: ${remaining}`);
  ...
  const targetCaloriesByDate = labels.map((date, index) => {
    const cumulativeTargetCalories = targets.reduce((acc, target) => {
      const targetDate = new Date(target.targetDate);
      const logDate = new Date(date);
      if (logDate <= targetDate) {
        acc += target.weeklyWeightLoss * 7700;
      }
      return acc;
    }, 0);
    return cumulativeTargetCalories;
  });
  ...
  return (
    ...
    >
      <h1 className="text-6xl font-extrabold mb-8 mt-20 overflow-visible animate-pulse bg-gradient-to-r from-purple-400 to-pink-600 text-transparent bg-clip-text">Reach Your Goals</h1>
      <div className="bg-white p-8 rounded-lg shadow-xl w-full max-w-md text-center text-black transform transition-transform duration-500 hover:scale-105 mb-8 overflow-visible">
        ...
        <div className="mb-4 overflow-visible">
          <h2 className="text-4xl font-extrabold mb-2 text-gradient bg-gradient-to-r from-red-500 to-yellow-500 text-transparent bg-clip-text">Remaining Calories</h2>
          <p className="text-gray-800 text-xl">{remainingCalories}</p>
        </div>
        {targetReached && (
